...
namespace Playground
{
    class Program
    {
        static public void CreateIndex(string indexName, string query)
        {
            bool indexExist = false;
            
            Db.Transact(() =>
            {
                indexExist = Db.SQL("SELECT i FROM Starcounter.Metadata.\"Index\" i WHERE Name = ?", indexName).Any();
            });
        ...
        static void Main()
        {
            string name = "IX_Item_Date";
            CreateIndex(name, $"CREATE INDEX {name} ON {nameof(Database.Item)} ({nameof(Database.Item.Date)})");
